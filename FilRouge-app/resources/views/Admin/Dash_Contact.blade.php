                ...
                <section class="grid grid-cols-3 gap-4 justify-center items-center px-2">
                    @foreach($contacts as $contact)
                        <div class="wrapper max-w-xs bg-gray-50 rounded-b-md shadow-lg overflow-hidden">
                            ...
                            <button data-modal-target="crud-modal{{ $contact->id }}" data-modal-toggle="crud-modal{{ $contact->id }}" data-email="{{ $contact->email }}" class="bg-green-900 w-full flex justify-center py-2 text-white font-semibold transition duration-300 hover:bg-green-800">
                                Contact The client
                            </button>
                        </div>

                        <!-- Contact the client -->
                        <div id="crud-modal{{ $contact->id }}" tabindex="-1" aria-hidden="true"
